  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mt-4">
        <input className="input" placeholder="New checklist name (e.g. Morning routine)" value={name} disabled={locked || loading === 'add'} onChange={(e) => setName(e.target.value)} />
        <button className="btn-primary" disabled={locked || loading === 'add'} onClick={addList}><span className="inline-flex items-center gap-2">{loading === 'add' && <Loader size="sm" />}{loading === 'add' ? 'Adding…' : 'Add'}</span></button>
      </div>
        ...
        {lists.map((l) => (
          <div key={l._id} className="card p-4">
            <div className="flex items-center justify-between">
              <input className="font-bold text-lg bg-transparent outline-none" value={l.name} disabled={locked || loading === `save_${l._id}`}
                onChange={(e) => mutate(l._id, (x) => { x.name = e.target.value; return x; })}
                onBlur={() => saveList(l)} />
              <div className="flex gap-1">
                <button className="btn-ghost px-3 py-1 text-xs" disabled={locked || loading === `save_${l._id}`} onClick={() => { mutate(l._id, (x) => { x.items = x.items.map((it) => ({ ...it, done: false })); return x; }); setTimeout(() => saveList(lists.find((x) => x._id === l._id)), 0); }}>{loading === `save_${l._id}` ? <Loader size="xs" /> : 'Reset'}</button>
                <button className="btn-danger px-3 py-1 text-xs" disabled={locked || loading === `del_${l._id}`} onClick={() => delList(l._id)}>{loading === `del_${l._id}` ? <Loader size="xs" /> : 'Delete'}</button>
              </div>
            </div>
